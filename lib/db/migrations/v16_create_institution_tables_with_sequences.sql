...
-- Migration v16: Create institution categories and types tables with sequences

-- Drop existing tables if they exist
DROP TABLE IF EXISTS institution_types CASCADE;
DROP TABLE IF EXISTS institution_categories CASCADE;

-- Create institution_categories table
...
-- Update existing foreign key references if they exist
DO $$
BEGIN
    -- Update institution_profiles table if it exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'institution_profiles') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'institution_profiles' AND column_name = 'institution_type_id') THEN
            ALTER TABLE institution_profiles 
            DROP CONSTRAINT IF EXISTS institution_profiles_institution_type_id_fkey;
    ...
    -- Update student_education_history table if it exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'student_education_history') THEN
        -- Add foreign key constraint if column exists
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_education_history' AND column_name = 'institution_type_id') THEN
            ALTER TABLE student_education_history 
            DROP CONSTRAINT IF EXISTS student_education_history_institution_type_id_fkey;
